...
| OD-5 | Keep or remove the academy "installment" option (ACA-6). **Removed from the UI in 3.2** so it no longer ships unbacked; choosing to keep it means building a payment plan | Phase 3 | Product owner |
...
| **OD-10** | **About page and headline figures.** Enter `established_year` and the `stat_*` figures in Site settings (all hidden while blank), confirm and publish team members (seeded inactive, no bios), add only verifiable awards, and confirm the remaining Our Story copy and 2019/2025 milestones in `StorySection.tsx`. Award milestones, "500 guests daily" and the invented stats were removed. | Before launch | Product owner |
