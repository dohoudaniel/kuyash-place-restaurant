...
from apps.common.exceptions import IdempotencyConflict, IllegalTransition
...
def test_a_different_body_under_the_same_key_is_a_different_request() -> None:
    """Keys are fingerprinted with the body, so a corrected retry is not a replay."""
    key = str(uuid.uuid4())
    first, _ = idempotency.begin("orders", key, {"payment_method": "card"})
    idempotency.complete(first, {"reference": "KYS-AAA111"})

    _, replay = idempotency.begin("orders", key, {"payment_method": "cash"})
    assert replay is None
